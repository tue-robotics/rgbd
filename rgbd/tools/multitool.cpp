...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    ...
    if (image) {
      ...
      if (depth.data) {
        cv::Mat depth_canvas(depth.rows, depth.cols, CV_8UC3,
                             cv::Scalar(50, 0, 0));
          ...
          for (int x = 0; x < depth.cols; ++x) {
            ...
            if (d > 0 && d == d) {
              unsigned char v = static_cast<unsigned char>(
                  std::min<float>(max_view_distance, d / max_view_distance) *
                  255);
        ...
        if (rgb.data) {
          ...
          cv::Mat rgb_roi = canvas(
              cv::Rect(cv::Point(0, 0), cv::Size(IMAGE_WIDTH, rgb_height)));
          cv::Mat depth_roi = canvas(cv::Rect(
              cv::Point(IMAGE_WIDTH, 0), cv::Size(IMAGE_WIDTH, depth_height)));

          cv::resize(rgb, rgb_roi, cv::Size(IMAGE_WIDTH, rgb_height));
          cv::resize(depth_canvas, depth_roi,
                     cv::Size(IMAGE_WIDTH, depth_height));
        } else {
          canvas = depth_canvas;
        }
      } else if (rgb.data) {
    ...
    if (!canvas.data) {
      ...
      cv::line(canvas, cv::Point(0, 0), cv::Point(640, 480),
               cv::Scalar(255, 255, 255), 5);
      cv::line(canvas, cv::Point(0, 480), cv::Point(640, 0),
               cv::Scalar(255, 255, 255), 5);
    }
    ...
    if (canvas.cols > IMAGE_WIDTH)
      cv::circle(canvas, mouse_pos + cv::Vec2i(IMAGE_WIDTH, 0), 5,
                 cv::Scalar(255, 0, 0), 1);

    cv::putText(canvas, MODE, cv::Point(10, 20), cv::FONT_HERSHEY_COMPLEX_SMALL,
                1, cv::Scalar(255, 255, 255), 1);

    if (PAUSE)
      cv::putText(canvas, "PAUSED", cv::Point(10, canvas.rows - 25),
    ...
    for (unsigned int i = 0; i < mouse_points.size(); ++i) {
      ...
      if (canvas.cols > IMAGE_WIDTH)
        cv::circle(canvas, mouse_points[i] + cv::Vec2i(IMAGE_WIDTH, 0), 5,
                   cv::Scalar(0, 0, 255), 2);
    }
